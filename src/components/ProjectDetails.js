...
//                 // If a supervisor is already assigned, fetch their full details
...
const ProjectDetails = ({ token }) => {
    ...
    //     try {
    //         const res = await fetch('http://localhost:8055/items/review', {
    //             method: 'POST',
    //             headers: {
    //                 'Content-Type': 'application/json',
    //                 Authorization: `Bearer ${token}`,
    ...
    // useEffect(() => {
    //     fetch(`http://localhost:8055/items/review?filter[project_id][_eq]=1&fields=id,comment,date_created,reviewer.id,reviewer.first_name,reviewer.last_name`, {
    //         headers: {
    //             Authorization: `Bearer ${token}`,
    //         },
    //     })
    ...
    return (
        <div className="p-4">
            ...
            {/* {user?.role?.name === 'supervisor' && user.id === project.project_supervisor && (
                <div className="mt-6">
            ...
            {/* {user?.role?.name === 'student' && (
                <div className="mt-6">
                    ...
                    {reviews.length === 0 ? (
                        ...
                        <ul className="space-y-3">
                            {reviews.map((review) => (
                                <li key={review.id} className="border p-3 rounded bg-yellow-50">
                                    <p>{review.review_text}</p>
                                    <p className="text-sm text-gray-600">
                                        By {review.reviewer?.first_name} {review.reviewer?.last_name} on {new Date(review.date_created).toLocaleString()}
                                    </p>
                                </li>
                            ))}
                        </ul>
